...
export async function createOrder(req, res) {
  try {
    ...
    const totalAmount = cart?.product?.reduce((total, item) => total + item.price * item.quantity,0);
    ...
    res
      .status(201)
      .json({ message: "Order created successfully", orderId: savedOrder._id });
  } catch (err) {
...
export async function singleOrder(req, res){
  try{
    ...
    if(orders.userId.toString() !== userId.toString()){
      return res.status(401).json({error: "User is not authorized to view this order"});
    } 
