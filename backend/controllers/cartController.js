...
export async function addToCart(req, res) {
  try {
    ...
    if (checkUserCart) {
      ...
      if ( checkUserCart.product.some(  (item) => item.productId.toString() === productId) ) {
        //increment quantity
        ...
        res.status(201).json({ message: "Product updated in cart", checkUserCart });
      } else {
          ...
          await checkUserCart.save();
          res.status(201).json({ message: "Product added in cart", checkUserCart });
      }
      } else {
